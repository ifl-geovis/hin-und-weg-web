/* ==========================================================================
   map.js — map initialization, layers, interaction, background switching
   ========================================================================== */

/* FIX: removed unused get_map_fit_padding() — superseded by get_map_fit_padding_options() */
...
/* Centralized fit-to-data with proper padding */
...
function map_labels(feature, layer)
{
	...
	let label_obj = L.marker(layer.getBounds().getCenter(), { icon: L.divIcon(label) });
...
function map_background_switcher()
{
	...
	/* Remove the current background layer if active */
	if (app.map.backgroundlayer && app.status.background_active) {
		app.map.backgroundlayer.removeFrom(app.map.map);
	}

	/* Advance index: 0 → 1 → ... → N-1 → -1 (off) → 0 → ...
	   FIX: removed unreachable branch that checked nextIndex === -1 && !background_active;
	   that condition can never be true because when nextIndex first becomes -1,
	   background_active is still true (we were showing the last option). */
	...
	/* Ensure correct layer ordering: background → data → selection */
