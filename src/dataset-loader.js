/* ==========================================================================
   dataset-loader.js — dataset selection, loading, CSV parsing
   ========================================================================== */

   function update_element_visibility() {
	let category_selector = document.getElementById("category_selector");
	category_selector.disabled = !app.selection.dataset_id;

	let load_dataset_button = document.getElementById("load_dataset_button");
	load_dataset_button.disabled = !(app.selection.dataset_id && app.selection.category_id);
...
function dataset_selected(event) {
	...
	document.title = app.selection.dataset.name || app.selection.dataset.title || "hin&weg web";
}
...
function create_centroid_mapping()
{
	...
	for (let feature of app.data.geodata.features)
	{
		...
		if (centroid && centroid.geometry && centroid.geometry.coordinates) {
			app.data.centroid_mapping[id] = [centroid.geometry.coordinates[1], centroid.geometry.coordinates[0]];
		}
	}
}

function load_migration_csv(results, file) {
	const match = /^.*year=([0-9]+)$/.exec(file);
...
function load_completed() {
	...
	/* Reflect defaults in UI after enabling controls */
	const theme_selector = document.getElementById("theme_selector");
	if (theme_selector) theme_selector.value = app.selection.theme;
	...
	if (swoopy_arrows_selector) swoopy_arrows_selector.checked = app.selection.swoopy_arrows;
	...
	/* FIX: removed duplicate year_selector.size = 10 block;
	   renew_year_selection() already sets it to min(years, 8) */
	...
	/* Preselect Berlin (DE3) if available */
